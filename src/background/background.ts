chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  console.log("request :");
  console.log(request);
  console.log(sender), sendResponse("Front the background script");
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  if (tab.url && tab.url.startsWith("https://mail.google.com/")) {
    if (changeInfo.status === "complete" && !injectScript) {
      ...
      const { avisoUserInfo } = await chrome.storage.local.get("avisoUserInfo");
      console.log("data", avisoUserInfo);
      chrome.scripting
        .executeScript({
